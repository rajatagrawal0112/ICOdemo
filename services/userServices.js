...
const addARTUser = async (email, password, name, mobile) => {
  try {
    ...
    var config = {
      method: 'post',
      url: 'https://theartw.com/api/artist-register',
      headers: { 
...
const updateARTPass = async (email, password) => {
  try {
    var data = JSON.stringify({
      "pass" : "toIbb5gvcKHCBvF1qr6hihMYU",
      "email" : email,
    ...
    var config = {
      method: 'post',
      url: 'https://theartw.com/api/password',
      headers: { 
        'Content-Type': 'application/json'
      },
      data : data
